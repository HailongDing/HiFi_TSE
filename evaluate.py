...
def generate_test_sample(target_wavs, interferer_pool, noise_files,
    ...
    target_wav = random_crop(target_wavs[indices[0]], seg_samples)
    ...
    for _ in range(num_interferers):
        ...
        if not int_spk_wavs:
            continue
        int_wav = random_crop(random.choice(int_spk_wavs), seg_samples)
    ...
    if rir is None:
        return None
    target_reverbed = apply_rir(target_wav, rir)
    interferers_reverbed = []
    for iw in interferer_wavs:
        r = get_rir()
        if r is not None:
            interferers_reverbed.append(apply_rir(iw, r))
